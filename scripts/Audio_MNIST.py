...
SYS_BATCH_SIZE =  int(sys.argv[2]) # 5
SYS_LEARNING_RATE = float(sys.argv[3])
SYS_LAYERS = int(sys.argv[4]) # 6
...
print(f"Seed: {SEED}")
print(f"Dimension_1: {DIM_1}")
print(f"Dimension_2: {DIM_2}")
print("\nAE:")
...
class MNISTAudioDataset(Dataset):
    def __init__(self, file_path, train=False, validation=False, test=False, dim_1=DIM_1, dim_2=DIM_2, seed=SEED):
        self.input_dimension = dim_1 * dim_2
        # Normalization and transform to tensor
        feature_transform = transforms.Compose([
            transforms.ToPILImage(),
            transforms.Resize((dim_1, dim_2)),
            transforms.ToTensor()
        ])
        ...
        print(f"original: {indices[:10]}")
        random.seed(seed)  # Set a seed for reproducibility (so that all shuffle are the same)
        random.shuffle(indices)
        print(f"shuffled: {indices[:10]}")
        ...
        if train:
            ...
            self.file_list = self.file_list[train_len:train_len+val_len]
            self.label_list = self.label_list[train_len:train_len+val_len]
        elif test:
            self.file_list = self.file_list[train_len+val_len:]
            self.label_list = self.label_list[train_len+val_len:]    
        ...
        for i in range(len(self.file_list)):
            ...
            spec = self.wav2melSpec(file_name) 
            spec = self.feature_transform(spec)
            spec = spec.reshape(-1, self.input_dimension) # flatten the tensor
        ...
        self.X = torch.squeeze(self.X, dim=1)
    ...
    def wav2melSpec(self, audio_path):
...
file_path = os.path.join(current_directory, '..', 'data','MNIST Audio', 'data')


mnist_audio_train = MNISTAudioDataset(file_path, train=True)
mnist_audio_validation = MNISTAudioDataset(file_path, validation=True)
mnist_audio_test = MNISTAudioDataset(file_path, test=True)


num_classes = mnist_audio_train.num_classes
input_dimension = mnist_audio_train.input_dimension
...
len(mnist_audio_train), len(mnist_audio_validation), len(mnist_audio_test)


# In[ ]:


X_train_max = torch.max(mnist_audio_train.X)
X_train_min = torch.min(mnist_audio_train.X)
X_test_max = torch.max(mnist_audio_test.X)
X_test_min = torch.min(mnist_audio_test.X)
...
mnist_audio_train.visualize_output_distribution()
mnist_audio_validation.visualize_output_distribution()
mnist_audio_test.visualize_output_distribution()
...
def img_MelSpectrogram(input, dim_1=DIM_1, dim_2=DIM_2, ax=None, label = None):
    # Reshape the input from torch.Size([784]) to torch.Size([28,28])
    input = input.reshape(dim_1, dim_2)
    
    spectrogram_dB = librosa.power_to_db(input, ref=np.max)
    ms_dB = np.squeeze(spectrogram_dB)  # Remove any extra dimensions

    if ax is None:
        fig, ax = plt.subplots()
    else:
        fig = ax.get_figure()  # Retrieve the figure from the existing axis

    
    img = librosa.display.specshow(spectrogram_dB, x_axis='time', y_axis='mel', ax=ax)
...
def visualize_output(output_list, images_per_epoch):
    for epoch_output in output_list:
        ...
        fig, axs = plt.subplots(2, images_per_epoch, figsize=(25,7))
        ...
        for i in range(images_per_epoch):
            ...
            img_MelSpectrogram(original_images[i], ax=ax, label=labels[i].item())

            # Plot the reconstructed image
            ax = axs[1, i]
            ax.set_title(f"Reconstructed {i+1}")
            img_MelSpectrogram(reconstructed_images[i], ax=ax, label=labels[i].item())
...
# Show Spectrogram for an example
# img_MelSpectrogram(mnist_audio_train.X[0])
...
AE_loss_values, visualize_list = train_and_validate_autoencoder(model = autoencoder, dataset_train=mnist_audio_train, dataset_validation=mnist_audio_validation, batch_size=BATCH_SIZE_AE, num_epochs=EPOCHS_AE, learning_rate=LEARNING_RATE_AE)

# Visualize 
# visualize_output(visualize_list, 10)

# Testing
AE_test_loss = test_autoencoder(autoencoder, dataset_test=mnist_audio_test, batch_size=BATCH_SIZE_AE)
...
encoded_input_train = autoencoder.encoder(mnist_audio_train.X)
encoded_input_train = encoded_input_train.detach()

encoded_input_validation = autoencoder.encoder(mnist_audio_validation.X)
encoded_input_valiation = encoded_input_validation.detach()

encoded_input_test = autoencoder.encoder(mnist_audio_test.X)
...
print(f"y_train.shape = {mnist_audio_train.y.shape}")


print(f"Minimum of X_train: {torch.min(mnist_audio_train.X)}")
print(f"Maximum of X_train: {torch.max(mnist_audio_train.X)}")
...
compressed_dataset_train = torch.utils.data.TensorDataset(encoded_input_train, mnist_audio_train.y)
compressed_dataset_validation = torch.utils.data.TensorDataset(encoded_input_validation, mnist_audio_validation.y)
compressed_dataset_test = torch.utils.data.TensorDataset(encoded_input_test, mnist_audio_test.y)
...
input_size_just_NN = mnist_audio_train.input_dimension
output_size_just_NN = mnist_audio_train.num_classes
...
NN_with_original_input_list =  train_and_validate_model(neural_network_just_NN, mnist_audio_train, mnist_audio_validation, BATCH_SIZE_ONLY_NN, EPOCHS_ONLY_NN, LEARNING_RATE_ONLY_NN)    


# Testing
NN_with_original_input_testing_list, NN_with_original_input_testing_loss, NN_with_original_input_testing_accuracy = test_model(neural_network_just_NN, dataset_test=mnist_audio_test, batch_size=BATCH_SIZE_ONLY_NN)
...
VQC_amplitude_list = train_and_validate_model(circuit_amplitude, mnist_audio_train, mnist_audio_validation, BATCH_SIZE_VQC_AMPLITUDE_EMBEDDING, EPOCHS_VQC_AMPLITUDE_EMBEDDING, LEARNING_RATE_VQC_AMPLITUDE_EMBEDDING) 


# Testing
VQC_amplitude_testing_list, VQC_amplitude_testing_loss, VQC_amplitude_testing_accuracy = test_model(circuit_amplitude, dataset_test=mnist_audio_test, batch_size=EPOCHS_VQC_AMPLITUDE_EMBEDDING)
...
# Define the models
models = [
    ('SEQUENT', SEQUENT(width=VQC_width, depth=LAYERS_SEQUENT), mnist_audio_train, mnist_audio_validation, mnist_audio_test, ['classical', 'quantum'], EPOCHS_SEQUENT, LEARNING_RATE_SEQUENT),
    ('DQC', DressedQuantumCircuit(width=VQC_width, depth=LAYERS_DRESSED), mnist_audio_train, mnist_audio_validation, mnist_audio_test, ['classical', 'quantum'], EPOCHS_DRESSED, LEARNING_RATE_DRESSED)
]
